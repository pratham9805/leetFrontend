...
function Login() {
  ...
  // ✅ Google OAuth — implicit flow (access_token sent to backend for verification via tokeninfo)
    ...
    <div className="min-h-screen flex flex-col lg:flex-row items-center justify-center relative overflow-hidden bg-linear-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center items-center lg:items-start px-4 sm:px-6 lg:px-12 py-8 lg:py-0 relative z-10">
        <div className="max-w-md w-full text-center lg:text-left">
          ...
          <h2 className="text-2xl sm:text-3xl lg:text-5xl font-bold text-white mb-6 leading-tight">
            Master Coding<br />One Problem<br />
            <span className="bg-linear-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">at a Time</span>
          </h2>
          <p className="text-sm sm:text-base lg:text-lg text-slate-300 mb-8 leading-relaxed">
            Join thousands of developers improving their problem-solving skills. Start your coding journey with us.
          </p>
          <div className="space-y-4 flex flex-col items-center lg:items-start">
            {['1000+ Problems to Solve', 'Real-time Leaderboard', 'Expert Community'].map((item) => (
              <div key={item} className="flex items-center gap-3">
                <div className="w-2 h-2 rounded-full bg-linear-to-r from-blue-400 to-purple-400 shrink-0"></div>
                <span className="text-xs sm:text-sm lg:text-base text-slate-300">{item}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="w-full lg:w-1/2 max-w-sm sm:max-w-md lg:max-w-lg px-4 sm:px-6 lg:px-12 py-8 lg:py-0 flex items-center justify-center relative z-10">
        <div className="w-full">
          <div className="mb-8">
            <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2 text-center lg:text-left">Login Account</h1>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-2.5 sm:py-3 px-4 rounded-lg bg-white text-slate-800 text-sm sm:text-base font-semibold shadow-md hover:shadow-lg hover:shadow-white/10 hover:bg-slate-50 active:scale-[0.98] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed border border-slate-200"
          >
            {googleLoading ? (
          ...
          {googleError && (
            <p className="text-red-400 text-xs sm:text-sm text-center mt-3">{googleError}</p>
          )}
        </div>
      </div>
    </div>
